...
`INSTA_OSS_TLS=custom` serves one certificate you supply for `*.<domain>`, for every name under
it, and issues nothing at all. No hostname is published, no order is placed, and renewal is yours:
the install page has the procedure and what the daemon does to make an approaching expiry hard to
miss.

...
```bash
ufw allow 22/tcp            # your SSH port FIRST: ufw defaults to DROP and will cut you off
ufw allow 80,443,5432/tcp
ufw enable
```

Allow SSH before you enable it, every time. An enable without it looks fine from the session that
ran it, because established connections survive, and locks you out on the next reconnect. The

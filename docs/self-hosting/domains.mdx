...
established connections survive, and locks you out on the next reconnect.

The installer never touches SSH rules, in either direction: your access is yours to preserve. It
prints this recipe with your detected ports filled in when it finds no active firewall, and the
rules it adds itself are only for the ports insta-oss serves. The

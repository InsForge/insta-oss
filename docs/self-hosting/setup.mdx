...
docker compose run --rm instad --reset-admin
docker compose start instad
```

The flag refuses to run while the daemon is up, because both processes would write the same state
file. It clears the admin record so `/setup` accepts a new one. Existing `insta_` tokens keep
working.

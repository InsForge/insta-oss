...
the same default. Postgres, and every service on the other branches, scale to zero. Every service
can be switched either way: compute and Postgres from the CLI, and any compute or managed database
...
that becomes always-on while it is asleep is started by the next sweep, within 30 seconds.
</Note>

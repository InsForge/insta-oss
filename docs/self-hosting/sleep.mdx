...
always-on, so production never waits for a cold start. Databases, and every service on the other
branches, scale to zero. Either can be switched per service, from the CLI, the API or the console.
...
<Note>
The defaults match the hosted platform: compute is always-on and scale-to-zero is opt-in, and
Postgres scales to zero. The one addition for a single box is that branch clones scale to zero
unless a service is explicitly always-on, so forking a project does not start every app it has.
Switch a service off with `insta compute always-on off web`, or turn everything back to
scale-to-zero with `INSTA_OSS_ALWAYS_ON_DEFAULT=0`.
</Note>
...
| `INSTA_OSS_ALWAYS_ON_DEFAULT` | 1 | compute on the default branch is always-on unless switched off; set to 0 to make it scale to zero too. Branch clones and Postgres scale to zero either way |

...
failing every branch create of an app that is awake. insta-oss copies. What the branch gets is the
state the app would find after a power cut, each file as it stood when the copy reached it. If a
group writes data that cannot take that, stop it first with `insta compute stop <group>`, or let
the idle sweep put it to sleep, and then create the branch.

...
Then, from a directory with a Dockerfile:
...
That's a running app on its own URL. The [quickstart](/quickstart) walks through each step, including wiring `DATABASE_URL` into your service and creating your first branch. If you're setting up a coding agent, `insta setup agent` wires the CLI, skill, and MCP server in one command — see [agent setup](/agents/setup).

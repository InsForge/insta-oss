...
InstaCloud is agent-native: the stack is designed so that everything you can do in the console, an agent can do through the `insta` CLI or an MCP tool call. These are the pieces that make it true, each with its own page:
...
- **Observe hook.** Linking a repo (`insta project create` or `insta project link`) adds a credential audit on agent tool calls that writes findings to `.insta/audit.jsonl`.

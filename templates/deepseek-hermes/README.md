...
This is the [Hermes Agent](../hermes/) template with DeepSeek set as the default model. It runs the
same official upstream image, [Hermes Agent](https://github.com/NousResearch/hermes-agent) by Nous
Research, with `HERMES_DEFAULT_MODEL` set to `deepseek/deepseek-chat` so you do not have to pick a
model after deploying. Everything else: the Telegram interface, the dashboard, the persistent
state: is identical.
...
- The Telegram usernames that should be allowed to use the bot.
...
`HERMES_HOME=/data/.hermes`, `HERMES_DASHBOARD=1`, and the dashboard bound to `0.0.0.0:8080`.

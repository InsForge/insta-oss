...
| `dsh` | yes | 401 unauthenticated, 200 authenticated | `ACCESS_PASSWORD` generated; Settings and Models verified in a browser, key stored through `credentials.set` | `/data` holds `DSH_HOME`, `HOME` and the workspace | Two processes in one container (nginx plus dsh), the tree's first. Needed three fixes a deploy found and a build could not: bubblewrap for the sandbox, the client-side loopback guard for the Models page, and a cookie for the WebSocket handshake. `GIT_TOKEN` was advertised and read by nothing; the Dockerfile now asserts every documented env name against the installed tree |

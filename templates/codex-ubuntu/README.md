...
Codex in an Ubuntu 24.04 browser workspace with extra compute and storage.

## Overview

This template runs [Codex](https://github.com/openai/codex), OpenAI's terminal coding agent,
inside an Ubuntu 24.04 LTS container that exposes a browser terminal. You open a URL,
authenticate, and get a `bash` shell with the `codex` CLI already installed. The larger compute
size and persistent disk provide more room for builds and repositories than the standard Codex
template.

The image is built from the Dockerfile in this directory: Ubuntu 24.04 LTS (pinned by digest),
Ubuntu's Node.js and npm packages, [ttyd](https://github.com/tsl0922/ttyd) 1.7.7 (verified against
a pinned SHA-256), and `@openai/codex` pinned to an exact version. Nothing floats on `latest`, so
a restart gives you the same environment.

## What you get by hosting it

- An HTTPS URL for the terminal, with no port forwarding or tunnel to manage.
- 2 vCPUs and 2 GiB of memory for builds and agent tasks.
- A 5 GiB volume mounted at `/data`. `HOME` is set to `/data/home`, so your CLI login, shell

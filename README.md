...
| `metrics` / `logs` | ✅ docker-backed (`docker stats` snapshot / `docker logs` tail — db logs included), cloud response shapes; targets `db\|compute\|redis\|mysql\|mongodb` — managed databases are their own component, per type (platform #243 parity); `logs --deploy` (deploy-event feed) → 501, use `insta events` |

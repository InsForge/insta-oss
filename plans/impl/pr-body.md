...
1. **The six in-repo template images are published amd64-only**, while the installer and docs promise aarch64, so six of eight bundled templates cannot deploy on an arm64 box. A CI fix is in the branch but unverified until that workflow runs, and the Dockerfiles may need cross-build work.

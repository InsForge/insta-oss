...
| DELETE /projects/:id/branches/:bid | gated branch.delete | 200 `{ teardown: { destroyed, failed } }`, or 409 with the same envelope plus `error` when `failed > 0` (decision 50 as amended) (today `{}`; existing assertion `test/server.test.ts:138` is WP5's allowed edit) | platform server.ts:1387 (`200: { teardown?: TeardownSummary }`); event `branch.delete` carries the summary | WP5 |

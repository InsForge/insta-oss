...
- `src/state.ts`: atomic tmp+rename write, sync-only assertion for `mutate()`, heartbeat lock file, coalesced `touchLater`, stat-keyed parse cache, `rev`, `initStatePath`. Regions for WP2 (`customDomains`, `lanes`) and WP5 (`templateDeployments`, `migrateState`) are left as marked stubs (`customDomains: {}`, `templateDeployments: {}`, `migrateState = (s) => s`).
...
- `src/server.ts` region A: `buildServer(engine, cfg = loadConfig())` (scaffold), `Fastify({ logger: false, trustProxy: cfg.trustProxy })`, delete the `/me` and `/tokens` 501 stubs the scaffold already moved INTO region A (contract 1.1) and call `registerAuth(app, cfg)` right after the content-type parser (line 33); the `// WP1` line of `API_PREFIXES` gains `/api`, `/auth`, `/tls` (WP5 owns the `/templates`, `/template-deployments` line); `uiDist` from `cfg.uiDist`; shell injection (below).
...
5. `// ---- region WP4 ----` (data dir probe + migration), `// ---- region WP5 ----` (template catalog), `// ---- region WP3 ----` (scheduler construction), `// ---- region WP2 ----` (router construction) are placeholder comments in that order; today's adapters are constructed with `cfg` (`new LocalGarage({ configPath: cfg.garageConfigPath, hostEndpoint: cfg.s3HostEndpoint })`).
6. `engine = new Engine(db, compute, storage, managed, { cfg })`; `app = buildServer(engine, cfg)`; `await app.listen({ host: cfg.listenHost, port: cfg.port })` (WP2 later replaces this with the router-owned server via `serverFactory`; the region comment marks the spot).
...
8. `SIGTERM`/`SIGINT`, in THIS order so shutdown fits the compose `stop_grace_period: 30s` and never leaves a fresh lock behind: `// region WP2: await router.stop()` first (stop accepting, destroy lane sockets, answer held wake requests 503 / ErrorResponse 57P03), then `// region WP3: await scheduler.stop()`, then `await Promise.race([app.close(), sleep(10_000)])` with Fastify's `forceCloseConnections: 'idle'` (in-flight requests and long-lived SSE/WebSocket/pg splices share the router's `node:http`/net servers, so an unbounded `close()` could exceed 30 s, get SIGKILLed, skip `releaseLock` and make the replacement container refuse the lock), then `releaseLock()`, `exit(0)`. `process.on('exit', releaseLock)`.

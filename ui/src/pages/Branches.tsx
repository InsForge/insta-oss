...
function NewBranchDialog({ projectId, from, onClose, onDone }: {
  ...
  const submit = async () => {
    ...
    if (r.kind === 'approval') return setError('branch creation gated — approve it from the Approvals page')
    onClose(); onDone(name.trim())
  }
  return (
    <Dialog title="New branch" onClose={onClose}>
      ...
      <p className="mt-3 text-xs text-neutral-500">
        A branch is a full isolated clone: its own Postgres (data copied), its own bucket (objects copied),
        and a redeploy of every app — nothing it does touches the source branch.
      </p>
      <ErrorNote error={error} />
      <div className="mt-5 flex justify-end gap-2">
        <Button kind="ghost" onClick={onClose}>Cancel</Button>
        <Button onClick={submit} disabled={busy}>{busy ? 'Cloning…' : 'Create branch'}</Button>
      </div>
    </Dialog>
  )
}
  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-10">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-neutral-900">Branches</h1>
      ...
      <div className="space-y-2">
        {(branches ?? []).map((b) => (
          <div key={b.id} className="group flex items-center rounded-xl border border-neutral-200 bg-neutral-50/60 px-4 py-3 hover:bg-neutral-50">
            ...
            <span className="ml-3">{b.is_default && <Chip>Production</Chip>}</span>
            <span className="ml-auto mr-4 text-xs text-neutral-400">{b.status}</span>
            {!b.is_default && (
              <button onClick={() => del(b.id)} title="delete branch (full teardown of its containers)"
      ...
      <p className="mt-6 text-xs text-neutral-400">
        Branches never merge — promote by merging code in git, running migration files against the target
        branch, and redeploying it (see the insta skill's branching guide).
      </p>
      <ErrorNote error={error} />
      {creating && (
        <NewBranchDialog projectId={projectId} from={(branches ?? []).map((b) => b.name)}
